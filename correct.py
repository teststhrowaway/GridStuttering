...
class MainApp(App):
    ...
    def show_image(self, widget, image, ext, time_delta):
        o = widget.height
        y = self.scroller.scroll_y
        h = self.pages_grid.minimum_height
        w = widget.width
        imw, imh = widget.set_image(image, ext)
        i = w * imh / imw
        assumed_new_height = h + i - o
        y_prime = (( y - 1 ) * h) / assumed_new_height + 1
        self.scroller.scroll_y = y_prime
